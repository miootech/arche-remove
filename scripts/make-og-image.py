...
Light theme variant to match the new default theme.
"""
import os
...
glow = mpatches.Ellipse(
    (200, -40), width=900, height=400,
    facecolor=AMBER, alpha=0.06, zorder=1
)
ax.add_patch(glow)
glow2 = mpatches.Ellipse(
    (1100, 50), width=500, height=300,
    facecolor=CREAM, alpha=0.04, zorder=1
)
ax.add_patch(glow2)
...
# Mini icon inside dark box: amber dot + white ring + diagonal line
...
ax.text(
    140, 75, "arche.remove",
    fontsize=22, color=TEXT, weight="bold",
    ha="left", va="center", zorder=6,
    family="DejaVu Sans"
)
ax.text(
    140, 100, "Local · Private · Free",
...
ax.text(
    60, 200, "Remove backgrounds.\nKeep everything else.",
    fontsize=64, color=TEXT, weight="bold",
    ha="left", va="top", zorder=6,
    family="DejaVu Sans",
    linespacing=1.05
)
ax.text(
    60, 350, "100% in your browser. No server uploads. No sign-up.",
...
for b in badges:
    ...
    box = FancyBboxPatch(
        (bx, 430), w, 44,
        boxstyle="round,pad=2,rounding_size=22",
...
# Right-side decorative: stylised "before/after" thumbnail
RIGHT_X = 800
THUMB_W = 360
THUMB_H = 360
THUMB_Y = 150
...
# Footer text
ax.text(
    60, 560, "Made with ♥ by Arche — arche-projects.pages.dev",
    fontsize=14, color=MUTED,
    ha="left", va="center", zorder=6,
    family="DejaVu Sans"
)
